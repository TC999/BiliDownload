...
static void
RSblock_initBlock(RSblock *block, int dl, unsigned char *data, int el, unsigned char *ecc) {
    block->dataLength = dl;
    block->data = data;
    block->eccLength = el;
    block->ecc = ecc;
...
static QRcode *QRcode_encodeStringReal(const char *string, int version, QRecLevel level, int mqr,
...
QRcode *QRcode_encodeStringMQR(const char *string, int version, QRecLevel level, QRencodeMode hint,
                               int casesensitive) {
    int i;
...
static QRcode *
QRcode_encodeDataReal(const unsigned char *data, int length, int version, QRecLevel level,
                      int mqr) {
    QRinput *input;
    QRcode *code;
    int ret;
...
QRcode *QRcode_encodeString8bitMQR(const char *string, int version, QRecLevel level) {
    ...
    for (i = version; i <= MQRSPEC_VERSION_MAX; i++) {
        QRcode *code = QRcode_encodeDataReal((unsigned char *) string, (int) strlen(string), i,
                                             level, 1);
        if (code != NULL) return code;
    }
...
QRcode_List *
QRcode_encodeDataStructured(int size, const unsigned char *data, int version, QRecLevel level) {
...
QRcode_List *QRcode_encodeString8bitStructured(const char *string, int version, QRecLevel level) {
    ...
    return QRcode_encodeDataStructured((int) strlen(string), (unsigned char *) string, version,
                                       level);
}

QRcode_List *
QRcode_encodeStringStructured(const char *string, int version, QRecLevel level, QRencodeMode hint,
                              int casesensitive) {
    if (string == NULL) {
        errno = EINVAL;
        return NULL;
    }
    return QRcode_encodeDataStructuredReal((int) strlen(string), (unsigned char *) string, version,
