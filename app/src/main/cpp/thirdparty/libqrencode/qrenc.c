...
static void
writeSVG_drawModules(FILE *fp, int x, int y, int width, const char *col, float opacity) {
    if (svg_path) {
...
static int writeSVG(const QRcode *qrcode, const char *outfile) {
    ...
    */
    ...
    if (svg_path) {
        if (fg_color[3] != 255) {
            fprintf(fp,
                    "\t\t<path style=\"stroke:#%s;stroke-opacity:%f\" transform=\"translate(%d,%d.5)\" d=\"",
                    fg, fg_opacity, margin, margin);
        } else {
            fprintf(fp, "\t\t<path style=\"stroke:#%s\" transform=\"translate(%d,%d.5)\" d=\"", fg,
                    margin, margin);
        }
    } else {
