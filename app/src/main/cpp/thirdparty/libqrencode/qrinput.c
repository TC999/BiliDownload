 ...
 */
STATIC_IN_RELEASE int
QRinput_insertStructuredAppendHeader(QRinput *input, int size, int number, unsigned char parity) {
    QRinput_List *entry;
    unsigned char buf[3];
...
static int QRinput_encodeModeNum(QRinput_List *entry, BitStream *bstream, int version, int mqr) {
    ...
    if (mqr) {
        ...
        ret = BitStream_appendNum(bstream, (size_t) MQRspec_lengthIndicator(QR_MODE_NUM, version),
        ...
        ret = BitStream_appendNum(bstream, (size_t) QRspec_lengthIndicator(QR_MODE_NUM, version),
                                  (unsigned int) entry->size);
        if (ret < 0) return -1;
    }
...
static int QRinput_encodeModeAn(QRinput_List *entry, BitStream *bstream, int version, int mqr) {
    ...
    if (mqr) {
        ...
        if (ret < 0) return -1;
        ret = BitStream_appendNum(bstream, (size_t) MQRspec_lengthIndicator(QR_MODE_AN, version),
                                  (unsigned int) entry->size);
        if (ret < 0) return -1;
    } else {
        ret = BitStream_appendNum(bstream, 4, QRSPEC_MODEID_AN);
        if (ret < 0) return -1;
        ret = BitStream_appendNum(bstream, (size_t) QRspec_lengthIndicator(QR_MODE_AN, version),
                                  (unsigned int) entry->size);
        if (ret < 0) return -1;
    }
...
static int QRinput_encodeMode8(QRinput_List *entry, BitStream *bstream, int version, int mqr) {
    ...
    if (mqr) {
        ...
        if (ret < 0) return -1;
        ret = BitStream_appendNum(bstream, (size_t) MQRspec_lengthIndicator(QR_MODE_8, version),
                                  (unsigned int) entry->size);
        if (ret < 0) return -1;
    } else {
        ret = BitStream_appendNum(bstream, 4, QRSPEC_MODEID_8);
        if (ret < 0) return -1;
        ret = BitStream_appendNum(bstream, (size_t) QRspec_lengthIndicator(QR_MODE_8, version),
                                  (unsigned int) entry->size);
        if (ret < 0) return -1;
    }
...
static int QRinput_encodeModeKanji(QRinput_List *entry, BitStream *bstream, int version, int mqr) {
    ...
    if (mqr) {
        ...
        if (ret < 0) return -1;
        ret = BitStream_appendNum(bstream, (size_t) MQRspec_lengthIndicator(QR_MODE_KANJI, version),
                                  (unsigned int) entry->size / 2);
        if (ret < 0) return -1;
    } else {
        ret = BitStream_appendNum(bstream, 4, QRSPEC_MODEID_KANJI);
        if (ret < 0) return -1;
        ret = BitStream_appendNum(bstream, (size_t) QRspec_lengthIndicator(QR_MODE_KANJI, version),
                                  (unsigned int) entry->size / 2);
        if (ret < 0) return -1;
    }
